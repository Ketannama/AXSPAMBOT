...
async def ping(e):
    if e.sender_id in SUDO_USERS:
        start = datetime.now()
        altron = await e.reply(f"𝐃𝐚𝐫𝐤 𝐗 𝐒𝐩𝐚𝐦")
        end = datetime.now()
        mp = (end - start).microseconds / 1000
        await altron.edit(f"𝐊ʏα 𝐎яƉɛя 𝐇αι 𝐌αяɛ 𝐌αƨтɛя")
...
async def stop(e):
    if e.sender_id in SUDO_USERS:
        await e.reply(f"`𝚂𝚃𝙾𝙿𝙸𝙽𝙶 𝐃𝐚𝐫𝐤 ʙᴏᴛ...`")
        try:
            await X1.disconnect()
        except Exception:
            pass
...
async def addsudo(event):
    if event.sender_id == OWNER_ID:
        ...
        ok = await event.reply(f"» __ᴀᴅᴅɪɴɢ 𝐃𝐚𝐫𝐤 𝐒𝐩𝐚𝐦 ʙᴏᴛ sᴜᴅᴏ....__")
        target = ""
        ...
        if str(target) in sudousers:
            await ok.edit(f"ᴛʜɪꜱ ᴜꜱᴇʀ ɪꜱ ᴀʟʀᴇᴀᴅʏ ᴀ 𝐃𝐚𝐫𝐤 𝐗 𝐒𝐩𝐚𝐦 ʙᴏᴛ ꜱᴜᴅᴏ ᴜꜱᴇʀ !!")
        else:
            if len(sudousers) > 0:
                newsudo = f"{sudousers} {target}"
            else:
                newsudo = f"{target}"
            await ok.edit(f"» **ɴᴇᴡ ꜱᴜᴅᴏ ᴜꜱᴇʀ**: `{target}`\n» `ᴡᴀɪᴛ ᴋᴀʀ ʙʜᴀɪ 𝐃𝐚𝐫𝐤 𝐗 𝐒𝐩𝐚𝐦 sᴜʀᴜ ʜᴏ ʀʜᴀ ʜᴀɪ...`")
